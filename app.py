...
<style>
:root{--bg:#0d1117;--s:#161b22;--s2:#21262d;--bd:#30363d;--ac:#58a6ff;--ac2:#7c5fe6;--gr:#3fb950;--re:#f85149;--tx:#c9d1d9;--mu:#8b949e}
*{margin:0;padding:0;box-sizing:border-box}
body{background:var(--bg);color:var(--tx);font-family:'Segoe UI',system-ui,sans-serif;min-height:100vh}
header{background:linear-gradient(135deg,#0d2137,#161b22);border-bottom:2px solid var(--ac);padding:16px 28px;display:flex;align-items:center;gap:12px}
...
.upload-zone{background:var(--s);border:2px dashed var(--bd);border-radius:14px;padding:44px 28px;text-align:center;cursor:pointer;transition:all .2s;position:relative;margin-bottom:20px}
...
.controls h3{font-size:12px;color:var(--mu);text-transform:uppercase;letter-spacing:1px;margin-bottom:12px}
.pills{display:flex;flex-wrap:wrap;gap:7px;margin-bottom:14px}
.pill{background:var(--s2);border:1px solid var(--bd);border-radius:20px;padding:5px 13px;font-size:12px;cursor:pointer;transition:all .15s;user-select:none}
...
input,select{width:100%;background:var(--s2);border:1px solid var(--bd);border-radius:7px;color:var(--tx);padding:9px 12px;font-size:13px;outline:none;transition:border-color .2s}
...
.th2{display:flex;justify-content:space-between;align-items:center;padding:14px 18px;border-bottom:1px solid var(--bd)}
...
.toast{position:fixed;bottom:20px;right:20px;padding:12px 20px;border-radius:9px;font-weight:600;font-size:13px;transform:translateY(70px);opacity:0;transition:all .3s;z-index:999;box-shadow:0 6px 20px rgba(0,0,0,.4)}
...
<div class="container">
  <div class="upload-zone" id="uz">
    ...
    <p>Drag &amp; drop or click to browse · Works 100% offline · No internet needed</p>
  </div>
  ...
  <div class="controls hidden" id="ctrl">
    <h3>🎯 Filter by Course ID — click to toggle (multi-select supported)</h3>
  ...
  <div class="exp hidden" id="ep">
    ...
    <button class="btn btn-p" onclick="exportXL()">⬇ Export to Excel (.xlsx)</button>
  </div>
</div>
...
function loadFile(f){
  ...
  r.onload=e=>{
    try{
      ...
      toast('✅ Loaded '+all.length+' records successfully!');
...
function buildPills(){
  ...
  ap.onclick=()=>toggle('__ALL__',ap);c.appendChild(ap);
  courses.forEach(cr=>{
    const p=document.createElement('span');
    p.className='pill';p.textContent=cr;p.dataset.c=cr;
    p.onclick=()=>toggle(cr,p);c.appendChild(p);
  });
  sel.clear();
...
function toggle(c,el){
  ...
  if(c==='__ALL__'){sel.clear();pills.forEach(p=>p.classList.remove('on'));el.classList.add('on')}
  else{
...
function resetF(){sel.clear();document.querySelectorAll('#pills .pill').forEach(p=>p.classList.remove('on'));document.querySelector('.pill.all').classList.add('on');document.getElementById('srch').value='';render()}
...
function render(){
  ...
  if(!f.length){tb.innerHTML='<tr><td colspan="'+(hdrs.length+1)+'" class="nd">No records match your filters.</td></tr>'}
  else{
    tb.innerHTML=f.map((r,i)=>{
      const cells=hdrs.map(h=>{
        ...
        if(String(h).toLowerCase().includes('mark')||String(h).toLowerCase().includes('obtain'))return'<td class="mc">'+v+'</td>';
        return'<td>'+v+'</td>';
      }).join('');
...
async function exportDoc(){
  ...
  try{
    ...
    const resp=await fetch('/generate_doc',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
    if(!resp.ok){throw new Error(await resp.text())}
    const blob=await resp.blob();
    const url=URL.createObjectURL(blob);
    const a=document.createElement('a');
    a.href=url;a.download='IGNOU_Award_List_'+courseCode+'.docx';a.click();
    URL.revokeObjectURL(url);
    toast('✅ Word file downloaded!');
  }catch(err){toast('Error: '+err.message,true)}
  finally{btn.disabled=false;btn.textContent='📄 Export IGNOU Award List (.docx)'}
}

function show(id){document.getElementById(id).classList.remove('hidden')}
function toast(m,isErr=false){
  const t=document.getElementById('toast');
  t.textContent=m;t.className='toast '+(isErr?'err':'ok');t.classList.add('show');
...
def fix_zoom(docx_bytes):
    """Fix missing w:percent attribute on w:zoom in settings.xml (python-docx bug)."""
...
def generate_doc():
    ...
    buf        = generate_award_list(candidates, course_code)
    raw_bytes  = buf.read()
    fixed      = fix_zoom(raw_bytes)

    filename = f'IGNOU_Award_List_{course_code}.docx'
    return send_file(
        io.BytesIO(fixed),
...
if __name__ == '__main__':
    print('\n' + '='*55)
    print('  IGNOU RC-71 Filter & Award List Generator')
    print('='*55)
